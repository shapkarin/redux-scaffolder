...
const checkAndCreate = async folderName => {
  try {
    const dir = path.join(process.cwd(), 'src', folderName);
    const exists = await statAsync(dir);
...
const createConstants = async function({
  ...
  try {
    // const exists = await checkAndCreate(folderName);
