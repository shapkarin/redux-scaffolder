...
const createSaga = async function({ take, constant }){
  try {
    const tpl = fs.readFileSync(path.join(__dirname, 'templates/saga.ejs'), 'utf-8');
    const result = render(tpl, { take, constant })

    await writeAsync(path.join(process.cwd(), "saga.js"), result, "utf-8");
    return true;
  } catch(e){
    return e;
  }
